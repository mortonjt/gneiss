...
class TestOLSCVKL(unittest.TestCase):
    ...
    def test_kfold_kl(self):
        ...
        exp = pd.DataFrame({
            'fold_0':[3584.576936, 0.168051, 663.109475, 1.092329],
            'fold_1':[3610.186080, 0.268887, 428.051996, 5.349238],
            'fold_2':[3768.107015, 0.199264, 197.383442, 0.393879],
            'fold_3':[3680.381545, 0.220822, 279.114696, 1.243997],
            'fold_4':[3021.728587, 0.288447, 960.836376, 1.892461],
            'fold_5':[3533.664042, 0.250359, 445.101004, 2.068317],
            'fold_6':[3390.104104, 0.220072, 581.636621, 2.792804],
            'fold_7':[3409.878710, 0.234888, 588.508539, 0.731105],
            'fold_8':[3369.763754, 0.168513, 709.649187, 8.859037]
            }, index=['model_mse', 'Rsquared',
                      'pred_mse', 'mean_KL']).T
        pdt.assert_frame_equal(res, exp)

    def test_kfold_kl_mask(self):
        np.random.seed(0)
        model = ols('x1 + x2', self.Y, self.X)
        model.fit()
        basis, _ = balance_basis(self.tree)
        ...
        Ymask = (np.random.rand(*ref_table.shape) > 0.1).astype(np.int)
        Ym = np.multiply(ref_table, Ymask)
        res = model.kfold(9, ref_table=Ym, tree=self.tree)

        exp = pd.DataFrame({
            'fold_0':[3584.576936, 0.168051, 663.109475, 1.201410],
            'fold_1':[3610.186080, 0.268887, 428.051996, 2.892000],
            'fold_2':[3768.107015, 0.199264, 197.383442, 0.694906],
            'fold_3':[3680.381545, 0.220822, 279.114696, 1.243997],
            'fold_4':[3021.728587, 0.288447, 960.836376, 1.670347],
            'fold_5':[3533.664042, 0.250359, 445.101004, 2.489060],
            'fold_6':[3390.104104, 0.220072, 581.636621, 3.961936],
            'fold_7':[3409.878710, 0.234888, 588.508539, 0.731105],
            'fold_8':[3369.763754, 0.168513, 709.649187, 6.382042]
            }, index=['model_mse', 'Rsquared',
                      'pred_mse', 'mean_KL']).T
        pdt.assert_frame_equal(res, exp)
